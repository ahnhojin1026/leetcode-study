...
3. if s[right] is already in char_lookup_set, remove s[left] and increment left pointer until no more repeating occure in char_lookup_set
4. Add the current character s[right] to the set.
5. Update the max_substr_len (r - l + 1) at each step.

## ⏱️ Complexity
* **Time Complexity:** $O(N)$
    * Reason: ceach character is added to the set at most once and removed at most once.
